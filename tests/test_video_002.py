...
class TestQualityCheckerVideo002:
    ...
    def test_fps_exact_24(self):
        qc = QualityChecker()
        assert qc.validate_fps(24.0) is True
        assert qc.validate_fps(23.98) is False
    ...
    def test_check_video_reports_short_duration_failure(self):
        ...
        try:
            ...
            assert builder.build_video([frame] * 24, output_path) is True
            ...
            assert results["video_info"]["fps"] == 24
            assert any("Duration:" in i and "FAIL" in i for i in results["issues"])
            assert any("FPS: 24" in i and "(OK)" in i for i in results["passed"])
        finally:
